...
try
{
    ...
    var wordsTask = Task.Run(() => CountWords(fileContent));
    var averageWordLengthTask = Task.Run(() => CalculateAverageWordLength(fileContent));
    var mostCommonWordsTask = Task.Run(() => GetMostCommonWords(fileContent, 5));
    var leastCommonWordsTask = Task.Run(() => GetLeastCommonWords(fileContent, 5));

    await Task.WhenAll(wordsTask, averageWordLengthTask, mostCommonWordsTask, leastCommonWordsTask);

    var words = wordsTask.Result;
    var averageWordLength = averageWordLengthTask.Result;
    var mostCommonWords = mostCommonWordsTask.Result;
    var leastCommonWords = leastCommonWordsTask.Result;
